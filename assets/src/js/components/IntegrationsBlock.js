 ...
 * @param {boolean}       props.adminBarEnabled      - Whether updatescontrol admin bar is enabled.
...
export function IntegrationsBlock({
	...
	return (
		<div className="updatescontrol-subcategory updatescontrol-subcategory-integrations">
			<hr />
			<h3>{__('Integrations', 'updatescontrol')}</h3>
			<SnippetToggleControl
				label={__(
					'Show updatescontrol admin bar button',
					'updatescontrol'
				)}
				value={adminBarEnabled}
				onChange={() => setAdminBarEnabled(!adminBarEnabled)}
				help={__(
					'Show a updatescontrol item in the admin bar with "Clear all caches" and sub-items for each active integration (page cache, static assets, object cache). Visible only when at least one of Cache Enabler, AutOptimize, or SQLite Object Cache is active. When enabled, those plugins\' own admin bar buttons are hidden.',
					'updatescontrol'
				)}
			/>
