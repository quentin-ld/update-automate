...
const PRESETS = [
	{
		...
		icon: '🖼️',
		title: __('Corporate website', 'updatescontrol'),
		description: __(
	...
	},
	{
		id: 'ecommerce',
		icon: '🛒',
		title: __('E-commerce', 'updatescontrol'),
		description: __(
...
export function PresetsSidebar({ onEnablePreset }) {
	return (
		<div className="updatescontrol-presets">
			<div className="updatescontrol-presets-description">
				<h2>{__('Pick configuration preset', 'updatescontrol')}</h2>
				<p>
					{__(
						"Don't know which features to enable? Quickly configure updatescontrol by selecting a preset that matches your site type. Each preset enables optimized features for your specific use case.",
						'updatescontrol'
					)}
				</p>
				{PRESETS.map((preset) => (
